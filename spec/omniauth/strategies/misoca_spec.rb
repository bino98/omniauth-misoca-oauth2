...
describe OmniAuth::Strategies::Misoca do
  ...
  let(:fresh_strategy) { Class.new(OmniAuth::Strategies::Misoca) }
  ...
  describe '#client' do
    ...
    it 'is initialized with dev_mode true' do
      instance = subject.new(app, dev_mode: true)
      expect(instance.client.options[:site]).to eq('https://dev.misoca.jp')
    end
  end
